...
yy73:
	...
	*p = YYCURSOR;
	...
	if (!strncmp((const char *)start + 2, "NAN", 3)) {
		*rval = Number::New(DoubleFromBits(0x7ff00000, 0x00000001));
	} else if (!strncmp((const char *)start + 2, "INF", 3)) {
		*rval = Number::New(DoubleFromBits(0x7ff00000, 0x00000000));
	} else if (!strncmp((const char *)start + 2, "-INF", 4)) {
		*rval = Number::New(DoubleFromBits(0xfff00000, 0x00000000));
	}
